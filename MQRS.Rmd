...
Here's some code to link right_ids to permits for just the multispecies fishery, courtesy of Dan C.:
```
SELECT PER_NUM AS PERMIT,
	  ...
	  WHERE FISHERY = 'MULTISPECIES'
		...
		AND (TRUNC(DATE_CANCELLED) >= '01-MAY-07' or DATE_CANCELLED IS NULL):
```
